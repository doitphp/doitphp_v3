 ...
 * @version $Id: View.php 3.1 2020-02-14 06:18:00Z tommy <tommy@doitphp.com> $
 * @package core
 * @since 1.0
 */
...
class View {
     ...
     * 加载并显示视图片段文件内容
     *
     * 相当于include 代码片段，当$isReturn为:true时返回代码代码片段内容,反之则显示代码片段内容。注:本方法不支持layout视图
     *
     * @access public
     *
     * @param string $fileName 视图片段文件名称
     * @param array $data 视图模板变量，注:数组型
     * @param boolean $isReturn 是否有返回数据。true:返回数据/false:没有返回数据，默认:false
     *
     * @return string
     */
    public function render($fileName = null, $data = array(), $isReturn = false) {
        ...
        if ($data && is_array($data)) {
            extract($data, EXTR_PREFIX_SAME, 'component');
            unset($data);
        } else {
            //当且仅当本方法在处理action视图(非视图片段)时，对本类assign()所传递的视图变量进行赋值
            if (!$fileName && $this->_options) {
                extract($this->_options, EXTR_PREFIX_SAME, 'action');
                $this->_options = array();
            }
        }

...
class Captcha {
    ...
    public function __construct() {
        ...
        $this->_fontSize     = 16;
        $fontNames           = array('aispec', 'yahoo', 'FetteSteinschrift', 'PilsenPlakat');
        $this->_fontName     = DOIT_ROOT . '/views/source/' . $fontNames[$_SERVER['REQUEST_TIME'] % 4] . '.ttf';
        $this->_sessionName  = 'doitphp_captcha_session_id';
        $this->_width        = 90;
        $this->_height       = 30;
        $this->_charts       = 'BCEFGHJKMPQRTVWXY2346789';
     ...
     * @return string
     */
    protected function _getCaptchaContent() {

        if (!$this->_textContent) {
            $num1 = $this->_charts[mt_rand(0, 23)];
            $num2 = $this->_charts[mt_rand(0, 23)];
            $num3 = $this->_charts[mt_rand(0, 23)];
            $num4 = $this->_charts[mt_rand(0, 23)];
    ...
    public function show($imageUrl = null) {
        ...
        if ($this->_imageUrl) {
            ...
            $backColor    = imagecolorallocate($this->_image, $bgColorArray[0], $bgColorArray[1], $bgColorArray[2]);
            imagefilledrectangle($this->_image, 0, 0, $this->_width -1, $this->_height - 1, $backColor);
        ...
        if (!preg_match('~[\x{4e00}-\x{9fa5}]+~u', $this->_textContent)) {
            ...
            for($i = 0; $i < $codeStrlen; $i ++) {
                ...
                $minY        = $fontH + 5;
                $maxY        = $this->_height -5;
                $fontY       = rand($minY, $maxY);

                $fontColor   = (!$this->_fontColor) ? imagecolorallocate($this->_image, mt_rand(1, 150), mt_rand(1, 150), mt_rand(1, 150)) : imagecolorallocate($this->_image, $this->_fontColor[0], $this->_fontColor[1], $this->_fontColor[2]);
        ...
        for ($i = 0; $i < 5; $i ++) {
            ...
            imageline($this->_image, mt_rand(2, $this->_width - 3), mt_rand(2, $this->_height - 3), mt_rand(2, $this->_width - 3), mt_rand(2, $this->_height - 3), $lineColor);
        }
        for($i = 0; $i < 9; $i ++) {
            $noiseColor = imagecolorallocate($this->_image, mt_rand(151, 254), mt_rand(151, 254), mt_rand(151, 254));
            imagestring($this->_image, 5, mt_rand(2, $this->_width - 3), mt_rand(2, $this->_height - 3), $this->_charts[mt_rand(0, 23)], $noiseColor);
        }

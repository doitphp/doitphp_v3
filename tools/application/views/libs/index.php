<fieldset class="mb60 padding20 pt15 pb50">
  ...
  <div class="padding5 pl15">
    <img src="<?php echo $baseAssetUrl; ?>/images/file_topdir.gif"> <a href="<?php echo $this->createUrl('files/index'); ?>/?path=/application/library" class="text-primary mr20">返回Library目录</a>
    <img src="<?php echo $baseAssetUrl; ?>/images/tree_folderopen.gif"> 当前目录：<span class="font-weight-bold"><?php echo $webappPath; ?>/application/library</span>
  </div>
  ...
  <div class="mt10 ml20 mr20">
    <div class="form-group form-inline">
      <label class="form-label font-weight-bold">Class 名称:</label>
      <input type="text" name="class_name" placeholder="请输入Class名称" class="input" id="class-name">
    </div>
  </div>
  <!--/基本信息-->
  ...
  <div class="mt40 text-center">
    <button type="submit" name="submit-btn" class="btn btn-success btn-lg">创建类文件</button>
  </div>
  <!--/提交按钮-->
  </form> 
</fieldset>
...
function createFileRequest(){
  ...
  if(className==''){
    showMsg("Class名称不能为空!", "danger");
    borderDanger($("#class-name"));
    return false;
  }
  return true;
}
...
$(function(){
  var className=$.cookie('filename');
  if(className){
    $("#class-name").val(className);
  }
  $("#class-name").on("blur", function(){

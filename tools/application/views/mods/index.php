<fieldset class="mb60 padding20 pt15 pb50">
  ...
  <div class="padding5 pl15">
    <img src="<?php echo $baseAssetUrl; ?>/images/file_topdir.gif"> <a href="<?php echo $this->createUrl('files/index'); ?>/?path=/application/models" class="text-primary mr20">返回Models目录</a>
    <img src="<?php echo $baseAssetUrl; ?>/images/tree_folderopen.gif"> 当前目录：<span class="font-weight-bold"><?php echo $webappPath; ?>/application/models</span>
  </div>
  ...
  <div class="mt10 ml20 mr20">
    <div class="form-group form-inline">
      <label class="form-label font-weight-bold">Model名称:</label>
      <input type="text" name="model_name" placeholder="请输入Model名称" class="input" id="model-name" style="width:240px;">

    <?php if(is_array($tableList) && $tableList){ ?>
      <input type="checkbox" name="bind_table_status" value="1" class="form-check-input ml15" id="bind-table-status">
      <label for="bind-table-status">绑定数据表(仅限MYSQL)</label>
      
      <div class="ml20 form-inline" id="bind-table-area" style="display:none;">
        <label class="form-label font-weight-bold">数据表名称:</label>
        <select name="table_name"  id="bind-table-name" class="input" style="width:180px;">
  ...
  <div class="mt40 text-center">
    <button type="submit" name="submit-btn" class="btn btn-success btn-lg">创建Model文件</button>
  </div>
  <!--/提交按钮-->
  </form> 
</fieldset>
...
function createFileRequest(){
  ...
  if(modelName==''){
    showMsg("Model名称不能为空!", "danger");
    borderDanger($("#model-name"));
    return false;
  }
  return true;
}
...
function ajaxFormResponse(json) {
  if(json.status==true){  
    ...
    layer.msg(json.msg, {icon:5,time:1000}, function(){      
      ...
      $("#model-name").val('');
      $.removeCookie('bindTableStatus');
      $("#bind-table-status").prop("checked", false);
      $("#bind-table-area").hide();
...
$(function(){
  var modsName=$.cookie('filename');
  if(modsName){
    $("#model-name").val(modsName);
  }

  var isBindTable = $.cookie('bindTableStatus');
  if(isBindTable=='true'){
    $("#bind-table-status").prop("checked", true);
    //show table area
    $("#bind-table-area").show();
  }
  ...
  $("#bind-table-status").on("click", function(){
    var isChecked=$(this).prop("checked");
    $.cookie('bindTableStatus', isChecked);
    //show table area
    if(isChecked == true){
      $("#bind-table-area").show();
    }else{
